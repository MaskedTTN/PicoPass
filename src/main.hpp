...
extern "C" void menu_task(void *params) {
    ...
    main_menu.add_item("BadUSB", badusb_selected);
    main_menu.add_item("Credentials", credentials_selected);
    main_menu.add_item("NFC Tools", nfc_selected);
    ...
    while (1) {
        ...
        if (main_menu.is_popup_active()) {
            ...
            // Check if developer mode is enabled and add LED Status if not already added
